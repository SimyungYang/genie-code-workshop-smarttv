...
## 원칙 1: 맥락 전환 시 새 대화 (Context Isolation)

> **가장 중요한 습관.** 하나의 대화에 **맥락이 다른 작업**을 섞으면 환각이 발생합니다.

Genie Code는 대화 내에서 "결정 트리"를 구축합니다. 완전히 다른 맥락의 작업이 섞이면 이전 결정이 간섭합니다.

### 같은 대화에서 이어가도 되는 경우

같은 테이블/같은 주제에 대한 연속 작업은 대화를 이어가는 것이 오히려 좋습니다. Genie Code가 앞에서 파악한 스키마와 컨텍스트를 재활용하기 때문입니다:

```
✅ 같은 대화 OK — 동일 테이블의 연속 작업:
"viewing_logs 분석해줘" 
→ "이 테이블을 정제하는 Silver 테이블을 만들어줘"
→ "방금 만든 Silver 테이블의 품질을 확인해줘"
```

### 새 대화를 시작해야 하는 경우

완전히 다른 맥락으로 넘어갈 때는 새 대화를 시작하세요:

```
❌ 하나의 대화에서 맥락 전환:
"viewing_logs EDA 해줘" → "Gold 테이블로 대시보드 만들어줘" → "Knowledge Assistant 만들어줘"
...
> 💡 **판단 기준**: "이전 대화에서 만든 결과물을 바로 이어서 쓰는가?" → 같은 대화. "완전히 새로운 제품/화면으로 넘어가는가?" → 새 대화.
