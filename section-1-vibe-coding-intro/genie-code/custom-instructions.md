...
```
@smart_tv.bronze.viewing_logs에서 지역별 시청 시간 합계를 구하고 차트로 보여줘.
```
...
## 네이밍 규칙
- 카탈로그: smart_tv
- 스키마: bronze(원본), silver(정제), gold(집계), quarantine(품질 위반 격리)
...
## MCP 도구 활용 (AI Dev Kit 연결 환경)
- Genie Space 생성/관리 → MCP manage_genie 도구 사용
- 대시보드 생성 → MCP manage_dashboard 도구 사용
- Job 스케줄링 → MCP manage_jobs 도구 사용
- Knowledge Assistant/Supervisor Agent → MCP manage_ka, manage_mas 도구 사용
- 노트북 코드 생성은 MCP 없이 직접 수행
- MCP 도구 호출 실패 시 재시도하고, 노트북 우회를 제안하지 말 것
```
...
```
@smart_tv.bronze.devices에서 region별 디바이스 수를 세줘. 10건만 보여줘.
```
