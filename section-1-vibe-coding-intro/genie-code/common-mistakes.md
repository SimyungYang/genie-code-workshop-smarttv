...
## 실수 1: 맥락이 다른 작업을 한 대화에서 섞기

| 상황 | 결과 |
|------|------|
| EDA → 대시보드 → 에이전트를 한 대화에서 | 컨텍스트 오염 → 환각, 잘못된 테이블 참조 |

> 같은 테이블의 연속 작업(분석 → 정제 → 검증)은 같은 대화 OK. **완전히 다른 맥락**으로 넘어갈 때 새 대화.

**해결**: 맥락이 바뀔 때 새 대화를 시작하세요. → [프롬프트 5대 원칙 — 원칙 1](prompt-principles.md) 참조
...
- **[보안 제한사항](security-limitations.md)** — 뭐가 되고 뭐가 안 되는지 미리 알면 삽질을 피할 수 있습니다.
